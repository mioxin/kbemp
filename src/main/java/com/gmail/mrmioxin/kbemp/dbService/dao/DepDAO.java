...
public class DepDAO implements IDao {
    ...
    public void dropDoubleRow() throws SQLException {
        ...
        executor.execUpdate("update " + TABLE + " a set a.parentname="
                + " (SELECT CONCAT(t1.name,'/',t2.name,'/',t3.name)" + "   FROM deps AS t1"
                + "   LEFT JOIN deps AS t2 ON t2.id=t1.parentid" + "   LEFT JOIN deps AS t3 ON t3.id=t2.parentid"
                + "   where a.parentid=t1.id)" + " WHERE a.parentname is null");
        ...
        executor.execUpdate("update " + TABLE + " set deleted=false where id not in " + "(SELECT max(b.id) FROM "
                + TABLE + " b GROUP BY b.name, b.parentname)");

        // перед удалением дубликатов с большим ID, проверить есть ли их ID где-то в
        // PARENTID
        // и заменить на id сохраненной копии
        executor.execUpdate("UPDATE " + TABLE + " v set v.parentid=(" + " CASE WHEN (select z.id FROM"
                + "         (SELECT* from " + TABLE + " where id in" + "                 (SELECT min(b.id) FROM "
                + TABLE + " b GROUP BY b.name, b.parentNAME)) as a," + " (SELECT* from " + TABLE + " where id not in"
                + "         (SELECT max(b.id) FROM " + TABLE + " b GROUP BY b.name, b.parentNAME)) as z,"
                + "  (SELECT* from " + TABLE + " where id not in" + "          (SELECT min(b.id) FROM " + TABLE
                + " b GROUP BY b.name, b.parentNAME)) as x"
                + "  WHERE a.parentid=x.id and x.name=z.name and x.parentname=z.parentname and V.ID=a.ID) is null"
                + "  THEN v.parentid" + "  ELSE  (select z.id FROM" + "          (SELECT* from " + TABLE
                + " where id in" + "                  (SELECT min(b.id) FROM " + TABLE
                + " b GROUP BY b.name, b.parentNAME)) as a," + "          (SELECT* from " + TABLE + " where id not in"
                + "                  (SELECT max(b.id) FROM " + TABLE + " b GROUP BY b.name, b.parentNAME)) as z,"
                + "  (SELECT* from " + TABLE + " where id not in" + "          (SELECT min(b.id) FROM " + TABLE
                + " b GROUP BY b.name, b.parentNAME)) as x"
                + "  WHERE a.parentid=x.id and x.name=z.name and x.parentname=z.parentname and V.ID=a.ID)" + "  END)");

        // удаление дубликатов с большим ID
        executor.execUpdate("delete from " + TABLE + " where id not in " + "(SELECT min(b.id) FROM " + TABLE
                + " b GROUP BY b.name, b.parentname)");
